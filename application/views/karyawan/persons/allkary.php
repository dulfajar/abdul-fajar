  ...
  <div class="content table-responsive table-full-width">
    <?php foreach ($raK as $rK):
      ...
      <div class="header">
        ...
        <p class="category"><?php echo round($rK->jumlah_kriteria,5); ?></p>
      </div>
      ...
      <tr>
        ...
        ?>
          ...
          <th hidden class="cal" id="R<?php echo $mkt.$rt.'x'.$da; ?>"><?php echo $dK->nilai_kriteria; ?></th>
        <?php endforeach; ?>
        <?php endforeach; ?>
...
<tr>
  ...
  <?php foreach ($listKar as $lK):
    ...
    $(document).ready(function(){
      ...
      var ntp = yA/xA;
      var bokr = <?php echo round($rK->jumlah_kriteria,5); ?>;
      var prkr = ntp*bokr;
  ...
  ?>
      <div id="realprint" class="card card-plain canvas_div_pdf">
        ...
        <table class="table table-hover table-striped">
          ...
          <tbody>
            <?php foreach ($fiRes as $fR): ?>
              <tr>
                <td hidden><input type="text" name="idK[<?php echo $fR->id_karyawan ?>]" value="<?php echo $fR->id_karyawan ?>" readonly> </td>
                <td><?php echo $fR->nama_karyawan ?></td>
                <td><?php echo $fR->jabatan ?></td>
                <td><input class="form-control" id="finRes<?php echo $fR->id_karyawan; ?>" type="text" name="totalakhir[<?php echo $fR->id_karyawan; ?>]" value="" readonly> </td>
              </tr>
              <script type="text/javascript">
              $(document).ready(function(){
                var sumC = 0;
        ...
        <table class="table table-hover table-striped">
        <tr>
          <td><input type="submit" class="btn btn-fill btn-info" name="" value="Simpan Ke Database"></td>
        </tr>
      </table>
      </div>
  </div>
