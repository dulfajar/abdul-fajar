...
<script type="text/javascript">
    $(document).ready(function(){
      $('#clk').load("<?php echo site_url('assessors/dataK'); ?>");
      $('.btnsend').click(function(event){
        event.preventDefault();
        var sk = $('.sorkar').val(); //Jbatan
        ...
        if ((sm=='<?php echo $dI->id_bagian ?>')&&(sk=='<?php echo $J->unique_jabatan; ?>')) {
          $('#clk').load("<?php echo site_url('assessors/dataKDJ/'.$J->unique_jabatan.'/'.$dI->unique_bagian); ?>");
        }
        else if((!sk)&&(sm=='<?php echo $dI->id_bagian ?>')) {
          $('#clk').load("<?php echo site_url('assessors/dataKD/'.$dI->unique_bagian); ?>");
        }
        <?php endforeach; ?>
        else if((sk=='<?php echo $J->unique_jabatan ?>')&&(!sm)) {
          $('#clk').load("<?php echo site_url('assessors/dataKJ/'.$J->unique_jabatan); ?>");
        }
        <?php endforeach; ?>
        else if((!sk)&&(!sm)) {
          $('#clk').load("<?php echo site_url('assessors/dataK'); ?>");
