  ...
  <div class="card">
    <div class="card card-plain">
      <div class="content table-responsive table-full-width">
        <div class="header">
          <h4 class="title">Ubah Prioritas Kriteria</h4>
        </div>
        <form class="" action="<?php echo base_url('welcome/showAbsen'); ?>" method="post">
        ...
        //tabel kriteria
            ...
            foreach ($data_absen as $dk)://foreach ambil dari Controller (buat Kriteria A)
              ...
              for ($i=1; $i<=$re ; $i++) {//for untuk mainkan logik decrease
                $jid= ($i);
                $jo=$jid;
                $this->db->select('nama_absen');
                $this->db->from('absen');
                $this->db->where('id_absen',$jo);
                $query=$this->db->get();
                $totab=$query->result();
              ...
              ?>
              <tr hidden>
                <td><input type="text" name="C[<?php echo $ra.$jid; ?>]" value="<?php echo $ra;?>"><?php echo $dk->nama_absen;?></td>
                <td><input type="text" name="W[<?php echo $ra.$jid; ?>]" value="1"> </td>
                <td><input type="text" id="" name="X[<?php echo $ra.$jid; ?>]" value="<?php echo $jo;?>"><?php echo $bc->nama_absen; ?></td>
            ...
            <?php
               ...
               <tr>
                 <td><input type="text" name="C[<?php echo $ra.$jid; ?>]" value="<?php echo $ra;?>"hidden><?php echo $dk->nama_absen; ?></td>
                 ...
                 <?php } ?></select></td>
                 <td><input type="text" id="" name="X[<?php echo $ra.$jid; ?>]" value="<?php echo $jo;?>"hidden><?php echo $bc->nama_absen; ?></td>
